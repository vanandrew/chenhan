...
bool CImprovedCHWithEdgeValve::CheckValidityOfWindow(Window &w)
{
	...
	if (m_InfoAtVertices[leftVert].disUptodate < 10000 / model.m_scale && m_InfoAtVertices[leftVert].disUptodate + w.proportions[1] * edge.length < w.disToRoot + rightLen)
	{
		return false;
	}
	...
	if (m_InfoAtVertices[rightVert].disUptodate < 10000 / model.m_scale && m_InfoAtVertices[rightVert].disUptodate + (1 - w.proportions[0]) * edge.length < w.disToRoot + leftLen)
	{
		return false;
	}
